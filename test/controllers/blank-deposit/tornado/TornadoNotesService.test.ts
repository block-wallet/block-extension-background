...
describe('Tornado', () => {
    describe('TornadoNotesService', () => {
        ...
        it('Should restore the deposits from the mnemonic', async () => {
            ...
            const depositsResult = await tornadoNotesService.reconstruct(
                mnemonic
            );
            ...
            let item = (deposit as PromiseFulfilledResult<IBlankDeposit[]>)
                .value;
            expect(item[0].note).to.be.equal(expectedDeposits[0].note);
            expect(item[1].note).to.be.equal(expectedDeposits[1].note);

            // ETH-10
            deposit = depositsResult[1];
            expect(deposit.status).to.be.equal('fulfilled');

            item = (deposit as PromiseFulfilledResult<IBlankDeposit[]>).value;
            expect(item.length).to.be.equal(0);
        }).timeout(10000);
        ...
        it('Should obtain 5 free deposit keys from the mnemonic without completing the iterator', async () => {
            ...
            for (let i = 0; i < 5; i++) {
                // Get next free deposit
                const { nextDeposit, recoveredDeposits, increment } =
                    await tornadoNotesService.getNextFreeDeposit({
                        amount: '1',
                        currency: KnownCurrencies.ETH,
                    });

                expect(typeof increment).not.to.be.equal('undefined');
                increment!();
                ...
                tornadoEventsDb.putValue('deposits-goerli-eth-1', {
                    commitment: last.deposit.commitmentHex,
                    leafIndex: 230,
                    timestamp: BigNumber.from('0x6032b002').toString(),
                    blockNumber: 123315,
                    transactionHash: '',
